...
3. **Ce dossier** — la liste des produits, et de quoi fabriquer la page et des
...
2. `python3 cafe/construire_page.py` — régénère `cafe/page/releve.html` ;
...
`cafe/page/apercu.html` est produit en même temps — c'est la même page, ouvrable
...
depuis la page, ou pour un relevé sans iPad. Depuis la racine du dépôt, le
script retrouve ses fichiers tout seul :

```bash
python3 cafe/alerte_stock.py --saisie                              # relevé produit par produit
python3 cafe/alerte_stock.py                                       # le rapport à l'écran
python3 cafe/alerte_stock.py --format html --sortie alerte.html    # page à imprimer
python3 cafe/alerte_stock.py --format feuille --sortie feuille.html  # relevé papier vierge
python3 cafe/alerte_stock.py --reinitialiser                       # semaine vierge
```

Aucune installation nécessaire : Python 3.11 ou plus récent suffit.

Le workflow [`alerte-stock.yml`](../.github/workflows/alerte-stock.yml)
...
```bash
python3 -m unittest discover -s cafe/tests -v
```
